...
class PageScrollView extends Component {
	...
	//官方设置的内置样式的各个参数
	builtinStyleArgs = {
		'sizeChangeMode': {sS: 0.7, oS: 0.7, bg: '#fff'},
		'rotateChangeMode': {rS: 45, skewS: 45, oS: 0.5, sS: 0.5, bg: '#fff'},
	};
	//官方设置的内置样式
	builtinStyles = {
		'sizeChangeMode': (index, data, {size, opacity}) => {
			...
			let widimage = customW || (w * 0.6), wid = isH ? widimage : viewWidth,
				hei = isH ? viewHeight : customH || (widimage / 16 * 9), heimage = customH || (widimage / 16 * 9);
			return (
				<View style={[{
					width: wid,
					height: hei,
		...
		'rotateChangeMode': (index, data, {rotate, opacity, size}) => {
			...
			let widimage = customW || (isH ? w * 0.5 : w * 0.7), wid1 = isH ? widimage : viewWidth,
				hei1 = isH ? viewHeight : customH || (widimage / 16 * 9), heimage = customH || (widimage / 16 * 9), hei = hei1,
				wid = wid1;
	...
	render() {
		...
		return (
			<View style={[style || {flex: 1}, builtinStyle && {backgroundColor: builtinStyleArgs[builtinStyle].bg}]}
	...
	renderPointer = () => {
		...
		for (let i = 0; i < datas.length; i++) {
			...
			viewArr.push(
				renderPointView ? <View key={i}>{renderPointView(selected)}</View> : <View key={i} style={[{
					width: 8,
					height: 8,
					borderRadius: 4,
					borderColor: pc[2],
					borderWidth: 1
				}, i && {marginLeft: 10}, selected ? {backgroundColor: pc[0]} : {backgroundColor: pc[1]}, this.props.pointerStyle]}/>
			);
		}
		return <View style={[{
			position: 'absolute',
	...
	renderScrollView = () => {
		...
		for (let i = 0; i < datas.length; i++) {
			...
			if (this.props.imageArr.length) {
				...
				if (builtinStyle) {
					arr.push(
						<TouchableOpacity activeOpacity={dealClick ? 0.5 : 1} onPress={() => {
					...
					arr.push(
						<TouchableOpacity activeOpacity={dealClick ? 0.5 : 1} onPress={() => {
							dealClick && dealClick(i % this.pNum)
						}} key={i} style={style} onLayout={this.imageLayout}>
				...
				arr.push(
					<View key={i} style={[]} onLayout={(event) => {
						(sS || oS || rS || skewS) ? this.setDistance1(event) : this.contentLayout(event)
					}}>
						{(sS || oS || rS || skewS) ? this.props.view(i % pageNum, datas[i % pageNum], {
							size,
							opacity,
							rotate,
							skew
	...
	scrollToPage = (currentPage, animated = true) => {
		...
		try {
			this.isH ? this.scrollView.scrollTo({
				x: currentPage * this.distance,
				animated: animated
		...
		if (this.props.ifInfinite) {
			...
			if (Platform.OS !== 'ios') {
				InteractionManager.runAfterInteractions(() => {
					this.scrollView.scrollTo(isH ? {
						x: this.state.pageNum * this.distance,
						animated: false
